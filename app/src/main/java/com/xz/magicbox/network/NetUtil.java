package com.xz.magicbox.network;

import com.orhanobut.logger.Logger;
import com.xz.magicbox.custom.OnLoadCompleteListener;
import com.xz.magicbox.custom.OnRequestListener;
import com.xz.magicbox.utils.IOStreamUtil;
    ...
    public void get_Sync(String url, OnRequestListener listener) {
        ...
        try {
            ...
            if (response.isSuccessful()) {
                ...
                listener.onSuccess(response.body().string(), response.headers().toString());

            } else {
                listener.onFailed(response.message(), response.code());
            }

        } catch (IOException e) {
            e.printStackTrace();
            listener.onFailed("连接失败："+url+"\n异常："+e.getMessage());
